...
class Fuse:
    ...
    def __init__(self, model_path: str):
        ...
        #quantize
        self.quantencoder = torch.quantization.quantize_dynamic(model=self.encoder,  # 原始模型
                                                                qconfig_spec={torch.nn.Linear, torch.nn.Conv2d},  # 要動態量化的NN運算元
                                                                dtype=torch.float16) # # 將權重量化為：float16 \ qint8

        self.quantencoder.load_state_dict(params['quantencoder'])
        self.decoder.load_state_dict(params['decoder'])

        self.quantencoder.to(device)
        self.decoder.to(device)
        
        self.quantencoder.eval()
        self.decoder.eval()
    ...
    def __call__(self, ir_folder: str, vi_folder: str, dst: str, fuse_type = None):
        ...
        para = sum([np.prod(list(p.size())) for p in self.quantencoder.parameters()]) + sum([np.prod(list(p.size())) for p in self.decoder.parameters()])
    ...
    def _forward(self, ir: torch.Tensor, vi: torch.Tensor) -> torch.Tensor:
        ir_en = self.quantencoder(ir)
        vi_en = self.quantencoder(vi)
...
if __name__ == '__main__':
    ...
    f(args.ir, args.vi, f'runs/NCHU_9k_float16/{model}')
